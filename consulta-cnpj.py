    ...
    while True:
        try:
            ...
            if response.status_code != 200:
                tqdm.write(f"[HTTP {response.status_code}] Erro. Aguardando {delay_atual}s...")
            ...
            if dados.get("status") != "OK":
                tqdm.write(f"[API ERRO] {cnpj}: {dados.get('message')} | Esperando {delay_atual}s...")
                time.sleep(delay_atual)
                delay_atual = min(delay_atual + 2, DELAY_MAX)
                continue
            ...
            tqdm.write(f"[ERRO DE REDE] {cnpj}: {e} | Esperando {delay_atual}s...")
            time.sleep(delay_atual)
            delay_atual = min(delay_atual + 2, DELAY_MAX)
...
def main():
    ...
    tqdm.write(f"✅ Coluna de CNPJ detectada: '{coluna_cnpj}'\n")

    resultados = []

    for _, row in tqdm(df.iterrows(), total=len(df), desc="Consultando CNPJs"):
